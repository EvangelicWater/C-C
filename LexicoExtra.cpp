    ...
    for ( int i = 0; i < contenedor.size(); ++i ) {
    	...
    	cout << "\n\n\n";
		 ...
		 if (validarLetra(contenedor[i])){
			...
			else if(validarEXAR(contenedor[i])){
				SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), 15); //Blanco
                        cout << " " << contenedor[i];
                        SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), 9); //Azul
                cout << " Es <exp-ar> ";
...
bool validarEXAR(const string& contenedor){
	regex EXAR{"((([A-Z][0-9]([a-z0-9]){1,8}\\.)|(\\d))\\s([+|-|*|/])\\s(([A-Z][0-9]([a-z0-9]){1,8}\\.)*|(\\d)*))\\."};
	return regex_match(contenedor,EXAR);
}
